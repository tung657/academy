...
import { CACHE_BLOG, useDeleteBlog } from '@/utils/query-loader/blog.loader';
...
export const BlogDelete = ({ label, id, path }: Props): JSX.Element => {
	const [opened, { open, close }] = useDisclosure();
	const t = useTranslations();

	const userRecoil = useRecoilValue(userState);

	const deleteQuery = useDeleteBlog({
		config: {
			onSuccess: async (data) => {
				...
				await queryClient.invalidateQueries([CACHE_BLOG.SEARCH]);
				handleCloseModal();
			},
			onError: (error) => {
	...
	return (
		<>
			...
			<ModalRender
				...
				title={t('blog-management.title_delete')}
				footer={{
					onOk: () => handleSubmit(),
