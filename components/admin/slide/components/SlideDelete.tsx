...
import { CACHE_SLIDE, useDeleteSlide } from '@/utils/query-loader/slide.loader';
...
export const SlideDelete = ({ label, id, path }: Props): JSX.Element => {
	const [opened, { open, close }] = useDisclosure();
	const t = useTranslations();

	const userRecoil = useRecoilValue(userState);

	const deleteQuery = useDeleteSlide({
		config: {
			onSuccess: async (data) => {
				...
				await queryClient.invalidateQueries([CACHE_SLIDE.SEARCH]);
				handleCloseModal();
			},
			onError: (error) => {
