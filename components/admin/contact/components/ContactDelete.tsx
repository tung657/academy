...
export const ContactDelete = ({ label, id }: Props): JSX.Element => {
	const [opened, { open, close }] = useDisclosure();
	const t = useTranslations();

	const userRecoil = useRecoilValue(userState);

	const deleteQuery = useDeleteContact({
		config: {
			onSuccess: async (data) => {
				...
				await queryClient.invalidateQueries([CACHE_CONTACT.SEARCH]);
				handleCloseModal();
			},
			onError: (error) => {
