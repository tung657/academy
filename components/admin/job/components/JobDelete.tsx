...
import { CACHE_JOB, useDeleteJob } from '@/utils/query-loader/job.loader';
...
export const JobDelete = ({ label, id, path }: Props): JSX.Element => {
	...
	const deleteQuery = useDeleteJob({
		config: {
			onSuccess: async (data) => {
				...
				await queryClient.invalidateQueries([CACHE_JOB.SEARCH]);
				handleCloseModal();
			},
			onError: (error) => {
