...
export const PartnerDelete = ({ label, id, path }: Props): JSX.Element => {
	const [opened, { open, close }] = useDisclosure();
	const t = useTranslations();

	const userRecoil = useRecoilValue(userState);

	const deleteQuery = useDeletePartner({
		config: {
			onSuccess: async (data) => {
				...
				await queryClient.invalidateQueries([CACHE_PARTNER.SEARCH]);
				handleCloseModal();
			},
			onError: (error) => {
